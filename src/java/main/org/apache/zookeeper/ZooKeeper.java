...
public class ZooKeeper {
    ...
    /**
     * 2会话创建过程：设置默认会话
     * 会创建一个客户端Watcher管理器保存在ClientWatherManager
     */
    ...
    public ZooKeeper(String connectString, int sessionTimeout, Watcher watcher,
        ...
        ConnectStringParser connectStringParser = new ConnectStringParser(
                connectString);
        /**
         * 3zookeeper会话创建过程：构造zookeeper服务器地址列表管理器
         * 对于构造方法中传入的服务器地址，客户端会将放在服务器地址列表管理器HostProvider
         */
        HostProvider hostProvider = new StaticHostProvider(
                connectStringParser.getServerAddresses());
        /**
         * 4zookeeper会话创建过程：创建初始化客户端网络连接器ClientCnxn
         * 用管理客户端和服务器网络交互
         * 创建一个服务端相应等待队列：pendingQueue
         * 客户端请求发送队列：outgoingQueue
         * 连接底层I/O处理器clientCnxnSocket
         */
